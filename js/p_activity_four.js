...
//        var selectTpl = template("t:selectCompany",{list:[{'companyName':'companyName1'},{'companyName':'companyName2'},{'companyName':'companyName3'},]})
//         $('#selectCompany').html(selectTpl)
    ...
    findGoods.wxFengxiang = function () {
        ...
        if (isWXBrowser()) {
            thJsCallWx.callWx({
                title: '焦易网—找船(车)篇！', // 分享标题
                desc: '货物已在手，没船(车)来运走？焦易来帮你！', // 分享内容
                link: window.location.href,//分享链接：当前页
                imgUrl: window.location.origin + '/images/activity/four.png' //分享时显示图片
            });
        }
    }
    findGoods.init()
